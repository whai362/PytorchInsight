...
def train(train_loader, model, criterion, optimizer, epoch, use_cuda):
    ...
    while inputs is not None:
        ...
        if batch_idx % args.print_freq == 0:
            ...
            #     bar.suffix = '({batch}/{size}) | Batch: {bt:.4f}s | Total: {total:.4f} | Loss: {loss:.4f} | top1: {top1: .4f} | top5: {top5: .4f}'.format(
            #         batch=batch_idx + 1,
            ...
            #     bar.next()
        if (batch_idx) % show_step == 0 and args.local_rank == 0:
            log = '({batch}/{size}) | Batch: {bt:.4f}s | Total: {total:.4f} | Loss: {loss:.4f} | top1: {top1: .4f} | top5: {top5: .4f}'.format(
                batch=batch_idx + 1,
                size=len(train_loader),
                bt=batch_time.avg,
                total=batch_time.sum,
...
def test(val_loader, model, criterion, epoch, use_cuda):
    ...
    while inputs is not None:
        ...
        # plot progress
        if args.local_rank == 0:
            bar.suffix = 'Valid ({batch}/{size}) | Batch: {bt:.4f}s | Total: {total:.4f} | Loss: {loss:.4f} | top1: {top1: .4f} | top5: {top5: .4f}'.format(
                batch=batch_idx + 1,
                size=len(val_loader),
                bt=batch_time.avg,
                total=batch_time.sum,
